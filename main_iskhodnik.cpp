...
menuStateType menuState=expectInput; //состояние обработки меню
...
MT16S2H_init();					//инициализация дисплея                
MT16S2H_on();					//включение дисплея
...
for(;;_delay_ms(TICK)){               //фоновая программа обрабатывается с периодичностью в 10мс
    ...
    case activate:              
        if(tempKey==longPress){               //если было длинное нажатие, то сохраняем будильник
            menuState=save;                   //выводим сообщение о сохранении результата                    
        }    
        else                                  //иначе продолжаем настраивать будильник
        if((tempKey!=noPress)||               //если было зафиксированна любое нажатие, то
           (tempEncoder!=notR)){
    ...
    case exitMenu:                  
        ...
        menuState=expectInput;                 //выходим из меню
    break;
    }   
}//for(;;)
return 0;
...
unsigned char txt[3][17]={"   WorldSkills  ",   //текст для вывода во второй строке
                          "     Russia     ",
                          "   Electronics  "};
